...
class CDGA_char2:
    ...
    def cycles(self):
        ...
        if self.QR:
            cycles = map(lambda c: self.QR.lift(c), cycles)
        return filter(lambda c: not c.is_zero(), cycles)
    ...
    def boundaries(self):
        ...
        if self.QR:
            boundaries = map(lambda b: self.QR.lift(b), boundaries)
        return filter(lambda b: not b.is_zero(), boundaries)
    ...
    def reduction_step(self):
        ...
        ker += g
        G = ker + b
        gb = self.R.ideal(G).groebner_basis()
        while True:
                ...
                while True:
                    while True:
                        ...
                        if r == (0, 0):
                            ...
                            q += (r[0] * r[1])
                        """
                        elif self.R.monomial_reduce(r[0], gb) == (0, 0): # how the hell does this work???
                            break
                        """
